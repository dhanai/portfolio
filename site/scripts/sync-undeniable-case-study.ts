...
const IMAGE =
  "https://xafmoppw6xwvpa6r.public.blob.vercel-storage.com/work/undeniable.webp";

async function main() {
  const study = caseStudies.find((c) => c.slug === "undeniable");
  if (!study) throw new Error("missing undeniable");

  const maxSort = await prisma.work.aggregate({ _max: { sortOrder: true } });
  const nextSort = (maxSort._max.sortOrder ?? 0) + 1;

  await prisma.work.upsert({
    where: { slug: "undeniable" },
    create: {
      slug: "undeniable",
      sortOrder: nextSort,
      published: true,
  ...
  console.log("undeniable case study upserted");
}
